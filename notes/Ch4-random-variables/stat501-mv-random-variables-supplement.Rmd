...
\begin{mdframed}[skipabove = 8pt, skipbelow = 8pt]
\begin{myresult}
If $\bs{\Sigma}_{22}$ is positive definite, then conditional on $\bm{y}_2 = \bm{\ddot{y}}_2$, $\bm{y}_1$ has the multivariate normal distribution
\[
...
where
$$
\bs\mu = \begin{pmatrix} \bs{\mu}_1 \\ \bm{A} \bs{\mu}_1 + \bm{b} \end{pmatrix} \hspace{1mm}\text{ and }\hspace{1mm}
\bs{\Sigma} = \begin{pmatrix} \bs{\Sigma}_{11} &  \bs{\Sigma}_{11}\bm{A}'\\ \bm{A}\bs{\Sigma}_{11} & \bs{\Omega} + \bm{A}\bs{\Sigma}_{11} \bm{A}' \end{pmatrix}.
$$ 
\end{myresult}
\end{mdframed}

...
library(pracma) # eye function
library(alr4) # data set
data(Rateprof)
```
...
and $Var(\bar{Y}_i | X)$.  
    ...
    \end{eqnarray*}
    _Assuming $Y_{ij}$ is independent of $Y_{ik}$ for $j \neq k$_,
    $$
...
the constant variance assumption? Explain why or why not.
    ...
    _No. The variance of $Y_i$ depends on $n_i$ (see previous derivation)._ 
...
in context of the problem.
    ...
    # Check with matrix algebra
    y <- Rateprof$quality
    n <- length(y)
    X <- matrix(cbind(rep(1, n), Rateprof$easiness, Rateprof$helpfulness), nrow = n, ncol = 3)
    Omega <- diag(1/Rateprof$numRaters)
    P <- X %*% solve(t(X) %*% solve(Omega) %*% X) %*% t(X) %*% solve(Omega)
    beta_hat <- P %*% y
    res <- (eye(n) - P) %*% y
    SSE <- (1/(n-3))*t(y) %*% solve(Omega) %*% (eye(n) - P) %*% y
    res_SE <- sqrt(SSE)
    ```

---
title: "M/STAT 501 Fall 2026: Homework 3"
subtitle: Due Wednesday, Oct 30 by 11:59pm in Gradescope
output: pdf_document
urlcolor: blue
---
...
1. Suppose the cdf of a discrete random variable $X$ with support $\mathcal{X} = \{0, 1,\ldots, 6\}$ has the value $F_X(x) = \dfrac{x(x+1)}{42}$ at the points $x = 0, 1, \ldots, 6$. 

    a. Find the pmf of $X$.
    b. Use R to plot a graph of the cdf of $X$, and a graph of the pmf of $X$. _Hint:_ One option for plotting in R is to set up an empty plot window using the `plot` function, and then add points and lines using the `points` and `lines` functions. For example:
        ```{r, out.width = '60%'}
...
in kb.
    ...
    b. Find the cdf $F(y)$. Show that $F(y)$ satisfies the three conditions of Theorem 1.5.3.
    c. Find the probability that the distance is between 3 and 7 kb using both the pdf and the cdf.
    d. What is $P(Y = 2)$?
    e. Use R to plot a graph of the cdf of $X$, and a graph of the pdf of $X$. _Hint_: Use the `curve` function.
